...
function requestPushPermissions() { if ("Notification" in window) { Notification.requestPermission().then(permission => { if (permission === "granted") alert("✅ Notificações ativadas! Vais receber alertas de descanso."); else alert("❌ Notificações recusadas."); }); } else { alert("O teu navegador não suporta Notificações Push."); } }
...
function checkSundayDebrief() { let today = new Date(); if (today.getDay() === 0) { let todayStr = today.toLocaleDateString('pt-PT'); if (lastDebriefDate !== todayStr) { let stats = generateSundayDebrief(); document.getElementById('debrief-vol').innerText = stats.volume.toLocaleString('en-US') + ' kg'; document.getElementById('debrief-work').innerText = stats.workouts; document.getElementById('sunday-debrief-modal').style.display = 'flex'; lastDebriefDate = todayStr; localStorage.setItem('gym_last_debrief', lastDebriefDate); setTimeout(() => { sendLocalPush("🛡️ Guardião de Dados", "É Domingo! Exporta o Backup na aba ID Cartão para não perderes o legado."); alert("🛡️ Guardião de Dados: Titã, é Domingo! Não te esqueças de ir à aba ID CARTÃO e clicar em 'Gravar .JSON' para exportares o teu Backup de segurança."); }, 1000); } } }
...
// --- MAPA DE CALOR, ANATOMIA E GRÁFICOS ---
...
function update1RMPrediction(exerciseName, maxWeight, maxReps, totalVolume) {
    ...
    if (best1RM > 0) { 
        ...
        let stdHtml = '';
        if(typeof getStrengthStandard === 'function') {
            let std = getStrengthStandard(exerciseName, best1RM, userProfile.weight, userProfile.gender);
            if(std) stdHtml = `<div class="strength-badge rank-${std.level.toLowerCase()}">Rank: ${std.level}</div><div style="font-size:11px; margin-top:5px;">Faltam ${std.nextKg}kg para o próximo nível.</div>`;
        }
...
function renderProfile() {
    ...
    history.forEach(log => {
        if(log.exercises) {
            Object.entries(log.exercises).forEach(([ex, sets]) => {
                let max = Math.max(...sets.map(s => parseFloat(s.weight||s.w||0)));
                let n = ex.toLowerCase();
    ...
    document.getElementById('sbd-total-val').innerText = sbdTotal + ' kg';
    let ratio = userProfile.weight > 0 ? (sbdTotal / userProfile.weight).toFixed(1) : 0;
    document.getElementById('sbd-ratio').innerText = `${ratio}x o teu peso corporal`;
...
function addGrocery() {
    let input = document.getElementById('grocery-input'); if(input.value.trim() === '') return;
    groceryList.push(input.value.trim()); localStorage.setItem('gym_groceries', JSON.stringify(groceryList));
    input.value = ''; renderGroceryList();
}
