...
// --- BIBLIOTECA DE TREINOS E DELOAD ---
...
function renderWorkout() {
    ...
    exercises.forEach((ex, exIdx) => {
        ...
        if(lastPerf && lastPerf.length > 0) { 
            let w = parseFloat(lastPerf[0].weight || lastPerf[0].w || 0);
            if(isDeloadMode && w > 0) preWeight = Math.round(w * 0.7); else preWeight = w || '';
...
function saveCurrentWorkout() {
    ...
    localStorage.removeItem('gym_active_session'); // Limpa backup
...
function promptCustomExercise() {
    let name = prompt("Nome do Exercício:"); if(!name) return; let muscle = prompt("Músculo Alvo (Peito, Costas, Pernas, Ombros, Braços, Core):"); if(!muscle) return;
    let newEx = { name: name, muscle: muscle.charAt(0).toUpperCase() + muscle.slice(1).toLowerCase(), tier: "A", type: "machine", defaultSets: 3 };
    customExercisesDB.push(newEx); exerciseLibrary.push(newEx); localStorage.setItem('gym_custom_exercises', JSON.stringify(customExercisesDB)); alert("✅ Adicionado ao Diretório!"); renderLibrary();
}
...
function updateHeatmap() {
    ...
    if(els.peito) {
        ...
        els.bicepsL.classList.toggle('muscle-pain-active', painTracker.includes('Cotovelos')); els.bicepsR.classList.toggle('muscle-pain-active', painTracker.includes('Cotovelos'));
    }
}
...
function update1RMPrediction(exerciseName, maxWeight, maxReps, totalVolume) {
    ...
    if (best1RM > 0) { 
        ...
        let stdHtml = '';
        if(typeof getStrengthStandard === 'function') { let std = getStrengthStandard(exerciseName, best1RM, userProfile.weight, userProfile.gender); if(std) stdHtml = `<div class="strength-badge rank-${std.level.toLowerCase()}">Rank: ${std.level}</div><div style="font-size:11px; margin-top:5px;">Faltam ${std.nextKg}kg para o próximo nível.</div>`; }
...
function triggerPunishment() { const cals = parseInt(document.getElementById('sin-cals').value); if (!cals || cals < 100) { alert('Mínimo 100kcal!'); return; } activePunishment = generatePunishmentLogic(cals); localStorage.setItem('gym_punishment', JSON.stringify(activePunishment)); closePunishmentModal(); renderPunishmentStatus(); alert('🔥 Tens uma penitência para pagar!'); let presetName = document.getElementById('sin-preset').options[document.getElementById('sin-preset').selectedIndex].text || "Pecado"; if(presetName === "Seleciona o Fast Food...") presetName = "Pecado / Cheat Meal"; dailyIntake.foods.push({ name: `⚠️ ${presetName}`, cals: cals, pro: parseInt(document.getElementById('sin-pro').value) || 0 }); localStorage.setItem('gym_daily_intake', JSON.stringify(dailyIntake)); renderDieta(); }
