...
function generateWorkout() {
    ...
    const fatigue = builderState.fatigue; builderState.routine = [];
    let targetMuscles = [];
    ...
    targetMuscles.forEach(muscle => {
        let pool = exerciseLibrary.filter(ex => ex.muscle === muscle); let selectedEx;
        if (fatigue === 'tired') {
            ...
            builderState.routine.push({ name: selectedEx.name, sets: Math.max(2, selectedEx.defaultSets - 1) });
        ...
        if (focus !== 'FULL' && (muscle === 'Peito' || muscle === 'Costas' || muscle === 'Pernas')) {
            ...
            if (secondEx) builderState.routine.push({ name: secondEx.name, sets: fatigue === 'tired' ? Math.max(2, secondEx.defaultSets - 1) : secondEx.defaultSets });
        }
    });
    ...
    updateBuilderUI(); alert('✨ Treino Inteligente gerado com sucesso!');
}
...
function renderSavedRoutines() {
    const list = document.getElementById('saved-routines-list'); list.innerHTML = '';
    if (savedRoutines.length === 0) { list.innerHTML = `<p class="text-center" style="color: var(--muted); padding: 20px 0;">Sem rotinas guardadas.</p>`; return; }
    savedRoutines.forEach((item, index) => {
        let totalSets = item.routine.reduce((sum, ex) => sum + parseInt(ex.sets), 0);
        list.innerHTML += `<div class="built-item" style="border: 1px solid #334155;"><div class="built-item-info"><span class="built-item-title" style="color: var(--accent);">${item.name}</span><span style="font-size: 11px; color: var(--muted);">${item.routine.length} Exercícios | ${totalSets} Séries</span></div><div class="built-item-controls" style="gap: 5px;"><button class="add-btn" style="width: auto; padding: 0 10px;" onclick="loadSavedRoutine(${index})">CARREGAR</button><button class="remove-btn" onclick="deleteSavedRoutine(${index})">✖</button></div></div>`;
    });
}
...
function updateWeeklyGoal() {
    ...
    history.forEach(log => {
        ...
        if(parts.length===3){ let d = new Date(parts[2], parts[1]-1, parts[0]); if (d >= sevenDaysAgo && d <= now) weekly++; }
    });
    document.getElementById('weekly-count').innerText = weekly; document.getElementById('weekly-progress').style.width = `${Math.min(weekly, 6) / 6 * 100}%`;
}
function updateMonthlyGoal() {
    ...
    history.forEach(log => { let parts = log.date.split('/'); if(parts.length===3 && parseInt(parts[2]) === year && parseInt(parts[1])-1 === month) monthly++; });
...
function updateProfileData() {
    ...
    document.getElementById('height-val').innerText = userProfile.height; document.getElementById('weight-val').innerText = userProfile.weight;
    ...
    tdee += (userProfile.gender === 'male') ? 5 : -161; tdee *= userProfile.activity;
    if (userProfile.goal === 'cut') tdee -= 500; if (userProfile.goal === 'bulk') tdee += 300;
...
function renderAchievements() {
    ...
    allAchievements.forEach(ach => {
        ...
        const filter = isUnlocked ? 'none' : 'grayscale(100%) opacity(0.3)';
        const color = isUnlocked ? 'var(--accent)' : 'var(--muted)';
        
        container.innerHTML += `
        <div style="display:flex; align-items:center; gap:15px; padding:12px; background:var(--bg-color); border-radius:12px; margin-bottom:10px; filter:${filter}; transition:0.3s;">
            <div style="font-size:30px; background:#1e293b; padding:10px; border-radius:50%; border:2px solid ${color};">${ach.icon}</div>
            <div>
                <h4 style="color:white; margin:0; font-size:15px;">${ach.title}</h4>
                <p style="color:var(--muted); font-size:12px; margin-top:3px;">${ach.desc}</p>
            </div>
        </div>`;
    });
}
...
function renderDieta() {
    ...
    let tdee = parseInt(calsElement.innerText) || 0; let weight = userProfile.weight; let goal = userProfile.goal;

    if (tdee === 0) return;
    let protein = Math.round(weight * 2.2); let fat = Math.round(weight * (goal === 'cut' ? 0.8 : 1.0));
...
function updateWearableData() {
    ...
    let waterGoalDisplay = document.getElementById('water-goal'); if (waterGoalDisplay) waterGoalDisplay.innerText = (baseWater + extraWater).toFixed(1) + ' L';
}
...
function renderPunishmentStatus() {
    ...
    if (!activePunishment) {
        container.innerHTML = `<p style="color:var(--muted); font-size:13px;">Estás limpo. Mantém a dieta.</p>`;
        ...
        container.innerHTML = `
            ...
            <div style="font-size:13px; color:white; line-height:1.6; margin-bottom:15px;">
                Para voltares a gravar treinos, tens de pagar:<br>
                • <b>${activePunishment.burpees}</b> Burpees<br>
                • <b>${activePunishment.squats}</b> Agachamentos c/ Salto<br>
                • <b>${activePunishment.pushups}</b> Flexões
            </div>
            <button class="beast-action-btn dropset" style="width:100%; padding:12px; background:var(--danger);" onclick="completePunishment()">🩸 PENITÊNCIA CUMPRIDA</button>
        `;
...
function completePunishment() {
    if(confirm('Tens a certeza que suaste isso tudo? Não mintas ao ferro.')) {
        activePunishment = null;
        localStorage.removeItem('gym_punishment');
        renderPunishmentStatus();
        alert('⛓️ Estás perdoado. Volta ao foco!');
    }
}
...
function openWarmup() {
    ...
    document.getElementById('warmup-results').innerHTML = `<div style="background: #334155; padding: 15px; border-radius: 12px; color: white; text-align: left; border-left: 4px solid #94a3b8;"><strong style="color: var(--accent);">Set 1:</strong> Barra (20kg) x 15 reps <br><span style="font-size: 11px; color: var(--muted);">Foco na técnica</span></div><div style="background: #334155; padding: 15px; border-radius: 12px; color: white; text-align: left; border-left: 4px solid #38bdf8;"><strong style="color: var(--accent);">Set 2:</strong> ${Math.round(targetW * 0.5)}kg x 8 reps <br><span style="font-size: 11px; color: var(--muted);">50% da carga alvo</span></div><div style="background: #334155; padding: 15px; border-radius: 12px; color: white; text-align: left; border-left: 4px solid #ef4444;"><strong style="color: var(--accent);">Set 3:</strong> ${Math.round(targetW * 0.75)}kg x 3 reps <br><span style="font-size: 11px; color: var(--muted);">75% da carga</span></div>`;
...
function openModoFlex() {
    ...
    if (exercises) {
        exercises.forEach((ex, exIdx) => {
            for (let setIdx = 1; setIdx <= 15; setIdx++) {
                let wInput = document.getElementById(`weight-${currentDay}-${exIdx}-${setIdx}`); let rInput = document.getElementById(`reps-${currentDay}-${exIdx}-${setIdx}`);
                if (wInput && rInput && wInput.value && rInput.value) { let w = parseFloat(wInput.value); let r = parseInt(rInput.value); if (!isNaN(w) && !isNaN(r)) { totalVolume += (w * r); totalSets++; } }
            }
        });
    }
    let username = userProfile.name ? '@' + userProfile.name.replace(/\s+/g, '').toLowerCase() : '@atleta_misterioso';
    document.getElementById('flex-card-name').innerText = username; document.getElementById('flex-card-date').innerText = new Date().toLocaleDateString('pt-PT');
    document.getElementById('flex-card-workout').innerText = currentDay.toUpperCase() + ' DAY'; document.getElementById('flex-card-volume').innerText = totalVolume.toLocaleString('en-US') + ' kg'; document.getElementById('flex-card-sets').innerText = totalSets + ' Sets';
    ...
    document.getElementById('flex-card-quote').innerText = `"${flexQuotes[Math.floor(Math.random() * flexQuotes.length)]}"`; document.getElementById('flex-modal').style.display = 'flex';
}
