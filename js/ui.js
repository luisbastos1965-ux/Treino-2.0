...
let radarInstance, bodyStatsInstance, tonnageInstance;
...
function renderWorkoutSlots() {
    ...
    savedRoutines.forEach((item, index) => {
        ...
        let actionBtn = deleteMode ? `<button class="info-btn" style="color:var(--danger); border-color:var(--danger);" onclick="deleteSavedRoutine(${index})">X</button>` : `<button class="info-btn" onclick="showWorkoutInfo('SAVED', ${index})">i</button>`;
    ...
    while(slotCount < minSlots) {
        container.innerHTML += `<div class="slot-container-flex"><div class="built-item empty-slot" style="flex:1; border:1px dashed #334155; background:transparent;"><div class="built-item-info"><span class="built-item-title" style="color:#64748b;">Slot Vazio</span></div></div><div style="width:55px; border-radius:12px; border:1px dashed #334155; background:transparent;"></div></div>`;
        slotCount++;
    }
...
function showWorkoutInfo(type, index) {
    ...
    if (type === 'TITAN') {
        title = 'Divisão Titã (PPL)'; totalSets = 52; 
        ...
        title = 'Mobilidade Activa'; totalSets = 10;
        content = `<p>Rotina regenerativa para dias de descanso. Foco na respiração e recuperação fluída das articulações e tendões.</p>`;
    } else if (type === 'SAVED') {
        const routine = savedRoutines[index]; title = routine.name;
        routine.routine.forEach(ex => totalSets += parseInt(ex.sets));
        content = `<ul style="padding-left:15px; margin:0;">`;
        routine.routine.forEach(ex => { content += `<li style="margin-bottom:5px;"><b>${ex.sets}x</b> ${ex.name}</li>`; });
        content += `</ul>`;
    }
...
function openWorkoutSlot(type, index = 0) {
    ...
    if (type === 'TITAN') {
        tabsContainer.style.display = 'flex'; tabsContainer.innerHTML = `<button class="tab-btn active" onclick="switchWorkout(event,'PUSH')">PUSH</button><button class="tab-btn" onclick="switchWorkout(event,'PULL')">PULL</button><button class="tab-btn" onclick="switchWorkout(event,'LEGS')">LEGS</button>`; currentDay = 'PUSH';
...
function renderWorkout() {
    ...
    if (currentDay === 'MOBILITY') {
        container.innerHTML = '<p style="color:var(--accent); font-size:13px; text-align:center; margin-bottom:15px;">Descomprime o Sistema Nervoso.</p>';
        exercises.forEach((ex, exIdx) => { container.innerHTML += `<div class="exercise-card" style="border-left: 4px solid var(--accent);"><div class="exercise-name">${ex.name}</div><div style="font-size: 13px; color: var(--muted); margin-bottom: 10px;">Execução: ${ex.sets} séries de ${ex.reps}</div><button class="beast-action-btn superset" style="width:100%; padding:10px;" onclick="this.innerText='✔ Concluído'; this.style.background='var(--success)'">Marcar Feito</button></div>`; }); return;
    }
    exercises.forEach((ex, exIdx) => {
        let html = `<div class="exercise-card"><div class="exercise-name">${ex.name}</div><div class="exercise-buttons"><button class="exercise-tip-btn" onclick="showExerciseTips('${ex.name}')">💡 Dica</button><button class="exercise-video-btn" onclick="openModal('${ex.name}', 'Consulta a execução.')">🎥 Vídeo</button></div>`;
        for (let i = 1; i <= ex.sets; i++) { html += `<div class="set-row" id="row-${currentDay}-${exIdx}-${i}"><span style="font-weight:bold; color:var(--muted); width:20px;">${i}</span><div class="input-group"><label>KG</label><input type="number" id="weight-${currentDay}-${exIdx}-${i}"></div><div class="input-group"><label>Reps</label><input type="number" id="reps-${currentDay}-${exIdx}-${i}"></div><div class="input-group"><label>RIR</label><select id="rir-${currentDay}-${exIdx}-${i}"><option value="0">0</option><option value="1" selected>1</option><option value="2">2</option><option value="3">3+</option></select></div><button class="check-btn" onclick="this.parentElement.classList.toggle('done')">✔</button></div>`; }
        html += `</div>`; container.innerHTML += html;
    });
}

function saveCurrentWorkout() {
    if(currentDay === 'MOBILITY') { alert('🧘‍♂️ Rotina concluída! O teu SNC agradece.'); return; }
    ...
    exercises.forEach((ex, exIdx) => {
        ...
        for (let i = 1; i <= (ex.sets + 10); i++) { 
            let w = document.getElementById(`weight-${currentDay}-${exIdx}-${i}`); let r = document.getElementById(`reps-${currentDay}-${exIdx}-${i}`); let rir = document.getElementById(`rir-${currentDay}-${exIdx}-${i}`);
            if (w && r && w.value && r.value) setsDetails.push({ weight: parseFloat(w.value), reps: parseInt(r.value), rir: rir.value });
        }
    ...
    updateHeatmap(); calculateRPGStats(); if(typeof checkAchievements === 'function') checkAchievements(); backToWorkoutSlots();
}
...
function generateWorkout() {
    ...
    if(typeof generateWorkoutLogic === 'function') { builderState.routine = generateWorkoutLogic(focus, fatigue, exerciseLibrary); updateBuilderUI(); alert(`✨ Treino gerado: ${builderState.routine.length} exercícios adaptados!`); }
}
...
function updateBuilderUI(rebuildList = true) {
    const list = document.getElementById('builder-routine-list'); const badge = document.getElementById('workout-volume-badge'); const actionBtns = document.getElementById('builder-action-buttons');
...
function applyBuiltWorkout() {
    ...
    document.getElementById('treino-slots-view').style.display = 'none'; document.getElementById('treino-active-view').style.display = 'block'; document.getElementById('active-workout-tabs').style.display = 'none';
    currentDay = 'CUSTOM'; const beastBtn = document.getElementById('main-beast-btn'); if (beastBtn) beastBtn.style.display = 'block'; renderWorkout();
}
function saveCurrentRoutine() {
    ...
    savedRoutines.push({ name: routineName, routine: JSON.parse(JSON.stringify(builderState.routine)) }); localStorage.setItem('gym_saved_routines', JSON.stringify(savedRoutines)); alert('✅ Guardada com sucesso!'); builderState.routine = []; updateBuilderUI(); navigateTo('view-treino');
}
...
// ==========================================
// MAPA DE CALOR, GRÁFICOS E ESTATÍSTICAS AVANÇADAS
// ==========================================
...
function updateHeatmap() {
    ...
    history.forEach(log => {
        ...
        if(parts.length === 3) {
            ...
            if (logDate >= sevenDaysAgo && logDate <= now) { if(log.exercises) Object.entries(log.exercises).forEach(([exName, sets]) => { let muscle = getMuscleForExercise(exName); if (volume[muscle] !== undefined) volume[muscle] += sets.length; }); }
        }
    });
...
function renderChart() {
    ...
    history.forEach(log => { if (log.exercises && log.exercises[exercise]) { labels.push(log.date); data.push(log.exercises[exercise][0].weight || log.exercises[exercise][0].w); reps.push(log.exercises[exercise][0].reps || log.exercises[exercise][0].r); } });
    if (chartInstance) chartInstance.destroy();
    const ctx = document.getElementById('progressChart').getContext('2d'); chartInstance = new Chart(ctx, { type: 'line', data: { labels, datasets: [{ label: 'Carga (kg)', data, borderColor: '#38bdf8', backgroundColor: 'rgba(56,189,248,0.2)', fill: true, tension: 0.3 }] }, options: { scales: { y: { beginAtZero: false } } } });
...
function updateGlobalStats() {
    ...
    history.forEach(log => { if(log.exercises) Object.entries(log.exercises).forEach(([exercise, sets]) => { if (!exercisesDone[exercise]) exercisesDone[exercise] = 0; exercisesDone[exercise]++; totalSets += sets.length; sets.forEach(set => totalVolume += (set.weight||set.w||0) * (set.reps||set.r||0)); }); });
...
function calculateRPGStats() {
    ...
    history.forEach(session => { if(session.exercises) Object.entries(session.exercises).forEach(([exName, setsDetails]) => { let volume = 0; setsDetails.forEach(set => { let w = parseFloat(set.weight || set.w); let r = parseInt(set.reps || set.r); if(w > 0 && r > 0) volume += (w * r); }); let muscle = getMuscleForExercise(exName) || categorizeMuscleByNameRPG(exName); if (muscleXP[muscle] !== undefined) muscleXP[muscle] += volume; else if (muscleXP['Braços'] !== undefined && (muscle === 'Bíceps' || muscle === 'Tríceps')) muscleXP['Braços'] += volume; }); });
...
// OS NOVOS GRÁFICOS (RADAR, TONAGEM, CORPO)
function renderAdvancedCharts() {
    // 1. Radar de Simetria
    let radarVol = { 'Peito': 0, 'Costas': 0, 'Pernas': 0, 'Ombros': 0, 'Braços': 0, 'Core': 0 };
    history.forEach(log => { if(log.exercises) Object.entries(log.exercises).forEach(([ex, sets]) => { let m = getMuscleForExercise(ex); sets.forEach(s => { if(radarVol[m] !== undefined) radarVol[m] += (s.weight||s.w||0) * (s.reps||s.r||0); else if(m==='Bíceps'||m==='Tríceps') radarVol['Braços'] += (s.weight||s.w||0) * (s.reps||s.r||0); }); }); });
    
    if (radarInstance) radarInstance.destroy();
    radarInstance = new Chart(document.getElementById('radarChart').getContext('2d'), {
        type: 'radar',
        data: { labels: Object.keys(radarVol), datasets: [{ label: 'Volume Acumulado (kg)', data: Object.values(radarVol), backgroundColor: 'rgba(56, 189, 248, 0.4)', borderColor: '#38bdf8', pointBackgroundColor: '#fff' }] },
        options: { scales: { r: { angleLines: { color: '#334155' }, grid: { color: '#334155' }, pointLabels: { color: '#94a3b8' }, ticks: { display: false } } }, plugins: { legend: { display: false } } }
    });

    // 2. Gráfico de Tonagem
    let tonHistory = {};
    history.forEach(log => {
        let parts = log.date.split('/'); if(parts.length!==3) return;
        let monthYear = `${parts[1]}/${parts[2]}`;
        if(!tonHistory[monthYear]) tonHistory[monthYear] = 0;
        if(log.exercises) Object.values(log.exercises).forEach(sets => sets.forEach(s => tonHistory[monthYear] += (s.weight||s.w||0) * (s.reps||s.r||0)));
    });
    // Pega nos últimos 6 meses registados
    let tonKeys = Object.keys(tonHistory).slice(-6);
    let tonData = tonKeys.map(k => tonHistory[k]);

    if (tonnageInstance) tonnageInstance.destroy();
    tonnageInstance = new Chart(document.getElementById('tonnageChart').getContext('2d'), {
        type: 'bar',
        data: { labels: tonKeys, datasets: [{ label: 'Tonagem (kg)', data: tonData, backgroundColor: '#22c55e', borderRadius: 6 }] },
        options: { scales: { y: { grid: { color: '#334155' } }, x: { grid: { display: false } } }, plugins: { legend: { display: false } } }
    });

    // 3. Gráfico de Evolução Corporal
    if (bodyStatsHistory.length > 0) {
        let dates = bodyStatsHistory.map(s => s.date.slice(0, 5)); // Só DD/MM
        let wData = bodyStatsHistory.map(s => s.weight);
        let rfmData = bodyStatsHistory.map(s => s.rfm);

        if (bodyStatsInstance) bodyStatsInstance.destroy();
        bodyStatsInstance = new Chart(document.getElementById('bodyStatsChart').getContext('2d'), {
            type: 'line',
            data: {
                labels: dates,
                datasets: [
                    { label: 'Peso (kg)', data: wData, borderColor: '#38bdf8', yAxisID: 'y', tension: 0.3 },
                    { label: 'RFM (%)', data: rfmData, borderColor: '#f59e0b', yAxisID: 'y1', tension: 0.3 }
                ]
            },
            options: {
                scales: {
...
function showHistoryDetails(dateString) {
    ...
    if (sessions.length === 0) { if(confirm("Registar treino vazio neste dia?")) { history.push({ date: dateString, day: 'MANUAL', exercises: {} }); localStorage.setItem('gym_tracker_history', JSON.stringify(history)); renderCalendar(); updateGlobalStats(); updateHeatmap(); } return; }
    let html = '';
...
function updateProfileData() {
    ...
    // Grava a evolução para o gráfico!
    ...
    if(existingStat) { existingStat.weight = userProfile.weight; existingStat.rfm = finalRfm; } 
    else { bodyStatsHistory.push({ date: todayStr, weight: userProfile.weight, rfm: finalRfm }); }
    localStorage.setItem('gym_body_stats', JSON.stringify(bodyStatsHistory));
