...
function renderDirectory() { const container = document.getElementById('directory-list'); container.innerHTML = ''; const groups = ['Peito', 'Costas', 'Pernas', 'Ombros', 'Braços', 'Core']; groups.forEach(muscle => { let pool = exerciseLibrary.filter(ex => ex.muscle === muscle); if (pool.length > 0) { let html = `<div class="dir-group"><h4>${muscle}</h4>`; pool.forEach(ex => { html += `<div class="dir-item" onclick="showExerciseTips('${ex.name}')" style="cursor:pointer;"><span>${ex.name}</span><span class="badge tier-${ex.tier.toLowerCase()}">${ex.tier}</span></div>`; }); html += `</div>`; container.innerHTML += html; } }); }
...
function clearBuilder() { if(confirm("Esvaziar o teu esboço atual?")) { builderState.routine = []; updateBuilderUI(); } }
function updateBuilderSets(index, value) { builderState.routine[index].sets = parseInt(value) || 1; updateBuilderUI(false); }
function updateBuilderUI(rebuildList = true) { const list = document.getElementById('builder-routine-list'); const badge = document.getElementById('workout-volume-badge'); const actionBtns = document.getElementById('builder-action-buttons'); let totalSets = 0; if (rebuildList) list.innerHTML = ''; if (builderState.routine.length === 0) { if (rebuildList) list.innerHTML = `<p class="text-center" style="color: var(--muted); padding: 20px 0;">Nenhum exercício selecionado.</p>`; badge.innerHTML = `0 Séries`; actionBtns.style.display = 'none'; return; } builderState.routine.forEach((item, idx) => { totalSets += parseInt(item.sets); if (rebuildList) { list.innerHTML += `<div class="built-item" style="display:flex; flex-wrap:wrap; gap:10px;"><div class="built-item-info" style="flex: 1; min-width: 150px;"><span class="built-item-title">${idx + 1}. ${item.name}</span></div><div class="built-item-controls" style="display:flex; gap:8px; align-items:center;"><span style="font-size: 10px; color: var(--muted);">SÉRIES</span><input type="number" class="set-input" value="${item.sets}" onchange="updateBuilderSets(${idx}, this.value)" style="width:40px;"><button class="remove-btn" style="background:transparent; color:#94a3b8; font-size:16px;" onclick="moveExerciseUp(${idx})">🔼</button><button class="remove-btn" style="background:transparent; color:#94a3b8; font-size:16px;" onclick="moveExerciseDown(${idx})">🔽</button><button class="remove-btn" style="background:rgba(239,68,68,0.2); color:var(--danger);" onclick="removeExerciseFromBuilder(${idx})">✖</button></div></div>`; } }); badge.innerHTML = `${totalSets} Séries <span onclick="clearBuilder()" title="Esvaziar" style="cursor:pointer; margin-left:8px; font-size:14px;">🗑️</span>`; badge.style.color = totalSets > 24 ? 'var(--danger)' : 'var(--accent)'; actionBtns.style.display = 'flex'; }
