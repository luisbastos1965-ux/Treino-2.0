...
function promptCustomExercise() { let name = prompt("Nome do Exercício:"); if(!name) return; let muscle = prompt("Músculo (Peito, Costas, Pernas, Ombros, Braços, Core):"); if(!muscle) return; let tier = prompt("Grau (S, A, B):"); if(!tier) tier = "B"; let newEx = { name: name, muscle: muscle.charAt(0).toUpperCase() + muscle.slice(1).toLowerCase(), tier: tier.toUpperCase(), type: "machine", defaultSets: 3 }; customExercisesDB.push(newEx); exerciseLibrary.push(newEx); localStorage.setItem('gym_custom_exercises', JSON.stringify(customExercisesDB)); showPulseToast("✅ Adicionado ao Diretório!"); renderLibrary(); }
